...
init();
// .. write down db
await fs.writeFileSync( "db/copy-tunnedDB.json", JSON.stringify( db, null, "\t" ) );
...
function init () {
    ...
    for( let x in db ) {
        ...
        // .. remove ravis
        if ( new_a != db[x].a ) { console.log( db[x].d ); db[x].a = new_a; }
    ...
    for ( let y in db ) {
        let tiny = "م ، قَالَ : «";
        if ( db[y].a.startsWith( tiny ) ) 
            db[y].a = db[y].a.replace( tiny, " " ).trim();

        db[y] = cs_finder_patch( db[y] );
    }
...
function cs_finder ( cell ) {
    ...
    for ( let cmd in c_map ) {
        matchPos = cell.a.slice( 0, c_map[ cmd ][1].length + 14 ).indexOf( c_map[ cmd ][1] );
        if ( matchPos > -1 ) {
            occur++; 
            pin = cmd; 
        }
    ...
    if ( occur >= 1 ) {
        ...
        let lID = cell.a.lastIndexOf( "»" );
        matched.a = cell.a.substring( fID, lID ).trim(); 
        matched.c = c_map[ pin ][0];
    }
...
function cs_finder_patch( cell ) {
    ...
    for ( let cmd of c_map ) {
        if ( cell.a.startsWith( cmd[1] ) )
            cell.a = cell.a.slice( cmd[1].length );
    }
