...
export function init () {
    ...
    let db: TS.db = JSON.parse( fs.readFileSync( "src/db/ready/وسائل‌الشيعة.json", "utf-8" ) );
    let exp: TS.db = JSON.parse( fs.readFileSync( "src/db/tmp/exp.json", "utf-8" ) );
    ...
    fs.writeFileSync( "src/db/ready/وسائل‌الشيعة.json", JSON.stringify( db, null, "\t") );
...
function main () {
    ...
    let base = fs.readFileSync( "src/db/tmp/base.html", "utf-8" );
    let box = base.split( "\n" );
    let mini_box: string[] = [];
    let big_box: string[][] = [];
    ...
    fs.writeFileSync( "src/db/tmp/exp.json", JSON.stringify( exported, null, "\t") );
    // fs.writeFileSync( "src/db/tmp/chcek.json", JSON.stringify( big_box, null, "\t") );
...
function patch () {
    ...
    parse();
    let file = fs.readFileSync( "src/db/tmp/hhh.html", "utf-8" );
    console.log(file.length);
    file = file.replace( /<span><\/span>/g, "" );
    file = file.replace( /\n/g, " " );
    file = file.replace( /<s/g, "\n<s" );
    file = file.replace( / +/g, " " );
    fs.writeFileSync( "src/db/tmp/hhh2.html", file );
...
function parse () {
    ...
    fs.writeFileSync( "src/db/tmp/hhh.html", BIG );
    ...
    // fs.writeFileSync( "src/db/tmp/xxx.html", BIG );
...
function loader ( num: number, type:"xxx"|"hhh" ) {
    return fs.readFileSync( "src/db/tmp/" + type + "_" + num + ".html", "utf-8" );
}
...
function pre_init () {
    ...
    fs.writeFileSync( "src/db/tmp/xxx_" + part + ".html", bigHTML );
