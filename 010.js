...
let filePath_kafi = "db/output/Al-Kafi_" + A_QC + ".json";
// .. check Al-Kafi
...
let filePath_misc = "db/output/Misc_" + X_QC + ".json";
// .. check Misc.
...
function init () {
    ...
    db_kafi = add_FLP( db_kafi );
    db_misc = add_FLP( db_misc );

    [ db_kafi, db_misc ] = wrapper_examine_X_by_A( db_kafi, db_misc );

    db_misc = finalizer ( db_misc )
    db_kafi = finalizer ( db_kafi )

    // .. write down db
    let exported_A = "db/output/Al-Kafi_" + A_QC + "__" + quality + ".json";
    fs.writeFileSync( exported_A, JSON.stringify( db_kafi, null, "\t" ) );
    // .. write down db
    let exported_X = "db/output/Misc_" + X_QC + "__" + quality + ".json";
...
function wrapper_examine_X_by_A ( A, X ) {
    ...
    for ( let i = 0; i < X.length; i++ ) {
        ...
        let j = 0;
        while ( j < A.length ) {
            let abd = Math.abs( A[j].inFaParts.length - X[i].inFaParts.length );
            if ( abd < 10 ) [ A, X ] = examine_X_by_A( j, i, A, X );
            j++;
        }
...
function examine_X_by_A ( AID, XID, A_db, X_db ) {
    ...
    if ( correlationRate > quality ) {
        ...
        outOne.qc = "i_" + (quality/100);

        let tmpX = outOne.x || [];
        delete outOne.x;
        if ( typeof pickedOne.x !== "object" ) pickedOne.x = [];
        let cont = [ ...pickedOne.x, ...tmpX, outOne ];
